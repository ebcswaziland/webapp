...
function ResultPage() {
    ...
    async function getOrdersData(country, state, city) {

        console.log("Stored Couwentry:", country);
        console.log("Stored Statwee:", state);
        console.log("Stored Citywe:", city);

        try {
            setLoading(true);


            let db_path = `/${country}/${state}/${city}`;
            setUrl(db_path)
            setUrlSecondary(`/${country}/${state}/Secondary`)
            console.log("Test10aaaaaaaaaa Poll " + db_path);
            console.log("Test10.1 Poll " + `${urlsecondary}/MP/nominees`);


            console.log("Test222222 Poll " + db_path);
            const product = await getDocs(collection(fireDB, `/${country}/${state}/Secondary/MP/nominees`));
    ...
    async function getPollingsData(country, state, city) {
        try {
            setLoading(true);


            const product2 = await getDocs(
                collection(fireDB, `/${country}/${state}/${city}/Pollings/stations`)
            );
    ...
    async function getTinkhundlaData(country, state, city) {
        try {
            setLoading(true);


            const productData = await getDocs(
                collection(fireDB, `/${country}/${state}/Secondary/Indvuna/nominees`)
            );
            const productsArray = [];
    ...
    async function getBucophoData(country, state, city) {
        try {
            setLoading(true);


            const productData = await getDocs(
                collection(fireDB, `/${country}/${state}/${city}/Bucopho/nominees`)
            );
            const productsArray = [];
            productData.forEach((doc) => {
                const obj = {
    ...
    const updateMP = async (product, pollstation) => {
        ...
        const mpDocRef = doc(fireDB, `/${country}/${state}/Secondary/MP/nominees`, product.id);
        try {
    ...
    const updatetindvuna = async (product, pollstation) => {
        ...
        const indvunaDocRef = doc(fireDB, `/${country}/${state}/Secondary/Indvuna/nominees`, product.id);
        try {
            await runTransaction(fireDB, async (transaction) => {
    ...
    const updateBucopho = async (product, pollstation) => {
        ...
        const mpDocRef = doc(
            fireDB,
            `/${country}/${state}/${city}/Bucopho/nominees`,
            product.id
        );
        try {
            await runTransaction(fireDB, async (transaction) => {
